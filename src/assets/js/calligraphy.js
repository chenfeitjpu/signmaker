(function() {
            ...
            , U = r => {
                ...
                var g = parseFloat(nr.value)
                  // 1. Apply natural log (n) to c, then divide by exp(g) 
                  // This scales down c by a factor determined by the slider value g
                  , c = h(n(c), N(g))
                  // 2. Apply softmax (d) to f to get probabilities, then take log (l)
                  // This converts raw scores to log probabilities
                  , f = (f = l(d(f)),
                s(f, 1 + g));
                for (let r = 0; r < K(f); r++)
            ...
            }, Z = null, $ = null, rr = null, er = null, tr = [], ar = document.getElementById.bind(document), lr = ar("canvas"), or = ar("select-style"), nr = ar("bias-slider"), vr = ar("speed-slider"), ir = ar("width-slider");
            ir.oninput = () => (r => {
                for (; lr.lastChild; )
                    lr.removeChild(lr.lastChild);
                if (0 != K(r)) {
            ...
            ur = "https://cdn.signmaker.io/signmaker/web/3n1/handwriting/d.bin",
            fr = ( () => {
                var r = 0
                  , e = [" ", ".", "..", "..", "..."]
